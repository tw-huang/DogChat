$(document).ready(function () {
    ...
    function load_messages() {
        ...
        if (position === 0 && socket.nsp !== '/anonymous') {
            ...
            $.ajax({
                url: messages_url, type: 'GET', data: {page: page}, success: function (data) {
    ...
    $('.delete-button').on('click', function () {
        var $this = $(this);
        $.ajax({
            type: 'POST', url: $this.data('href'), success: function () {
...
$(document).ready(function () {
    $('.login.ui.form').form({
        fields: {
            email: {
                identifier: 'email',
                rules: [{type: 'empty', prompt: 'Please enter your e-mail'}, {
                    type: 'email',
    ...
    $('.profile.ui.form').form({
        ...
        fields: {
            ...
            github: {identifier: 'github', optional: true, rules: [{type: 'url', prompt: 'Please enter a valid url'}]},
            website: {
                identifier: 'website',
                optional: true,
                rules: [{type: 'url', prompt: 'Please enter a valid url'}]
            },
            bio: {
                identifier: 'bio',
                optional: true,
                rules: [{type: 'maxLength[25]', prompt: 'Your bio must be not more than {ruleValue} characters'}]
            }
        }
    });
});